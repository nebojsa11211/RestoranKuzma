...
try
{
    ...
    var screenshotDir = @"D:\KimiTest\RestoranKuzma\scripts\screenshots";
    Directory.CreateDirectory(screenshotDir);

    await page.ScreenshotAsync(new PageScreenshotOptions
    {
        Path = Path.Combine(screenshotDir, "01-home-page.png"),
        FullPage = true
    });
    ...
    await page.ScreenshotAsync(new PageScreenshotOptions
    {
        Path = Path.Combine(screenshotDir, "02-before-click.png"),
        FullPage = true
    });
    ...
    await page.ScreenshotAsync(new PageScreenshotOptions
    {
        Path = Path.Combine(screenshotDir, "03-after-navigation.png"),
        FullPage = true
    });
    ...
    foreach (var selector in sidebarSelectors)
    {
        ...
        if (count > 0)
        {
            ...
            // Get sidebar content
            var sidebarText = await page.Locator(selector).TextContentAsync();
            if (sidebarText != null && sidebarText.Length > 0)
            {
                Console.WriteLine($"  Sidebar content preview: {sidebarText.Substring(0, Math.Min(200, sidebarText.Length))}...");
            }
            break;
        }
    }
    ...
    Console.WriteLine($"\n✓ Page body content length: {bodyContent?.Length ?? 0} characters");
    if (bodyContent != null && bodyContent.Length > 0)
    {
        Console.WriteLine($"  Content preview: {bodyContent.Substring(0, Math.Min(300, bodyContent.Length))}...");
    }
    ...
    if (consoleErrors.Count > 0)
    {
        ...
        foreach (var error in consoleErrors.Take(10))
        {
            Console.WriteLine($"  - {error}");
        }
        if (consoleErrors.Count > 10)
        {
            Console.WriteLine($"  ... and {consoleErrors.Count - 10} more errors");
        }
    }
    else
    {
    ...
    var testPassed = urlMatches && hasHeading && !hasDashboardContent;
...
catch (Exception ex)
{
    ...
    try
    {
        ...
        await page.ScreenshotAsync(new PageScreenshotOptions
        {
            Path = Path.Combine(screenshotDir, "error-screenshot.png"),
            FullPage = true
        });
